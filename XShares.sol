...
contract XSharesSharesV1 is Ownable { 
    ...
    function register(address sharesSubject,uint256 curveIndex) public payable{
        uint256 supply = sharesSupply[sharesSubject];
        require(supply == 0 || sharesSubject == msg.sender, "Only the shares' subject can register");  
        require(curveIndex >= 1 && curveIndex <= 10,"The curveIndex can only be between 1 and 10");
        uint256 price = getPrice(supply, 1 ,curveIndex);  
        uint256 protocolFee = price * protocolFeePercent / 1 ether;
        uint256 subjectFee = price * subjectFeePercent / 1 ether;
        require(msg.value >= price + protocolFee + subjectFee, "Insufficient payment");
        sharesBalance[sharesSubject][msg.sender] = sharesBalance[sharesSubject][msg.sender] + 1; 
        sharesSupply[sharesSubject] = supply + 1;
        sharesCurveIndex[sharesSubject] = curveIndex; 
        emit Trade(msg.sender, sharesSubject, true, 1, price, protocolFee, subjectFee, supply + 1);
        (bool success1, ) = protocolFeeDestination.call{value: protocolFee}("");
        (bool success2, ) = sharesSubject.call{value: subjectFee}("");
        require(success1 && success2, "Unable to send funds");
    }
